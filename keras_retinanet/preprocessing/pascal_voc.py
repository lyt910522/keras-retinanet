...
class PascalVocGenerator_part(Generator):
    ...
    def _get_image_names(self):
        image_names = []
        for temp_set_name in self.set_name_list:
            temp_names = [l.strip().split(None, 1) for l in open(os.path.join(self.data_dir, 'ImageSets', 'Main', temp_set_name + '.txt')).readlines()]
            for name, exist in temp_names:
                if int(exist) == 1:
    ...
    def __parse_annotation(self, element):
        ...
        return truncated, difficult, box, label, True
    ...
    def __parse_annotations(self, xml_root):
        ...
        for i, element in enumerate(xml_root.iter('object')):
            try:
                truncated, difficult, box, label, is_contain = self.__parse_annotation(element)
            except ValueError as e:
                raise_from(ValueError('could not parse object #{}: {}'.format(i, e)), None)
